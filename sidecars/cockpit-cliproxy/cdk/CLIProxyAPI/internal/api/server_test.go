...
func TestVideosRetrieveRouteUsesOpenAICompatibleHandler(t *testing.T) {
	server := newTestServer(t)

	for _, route := range server.engine.Routes() {
		if route.Method != http.MethodGet || route.Path != "/v1/videos/:video_id" {
			continue
		}
		if !strings.Contains(route.Handler, "VideosRetrieve") {
			t.Fatalf("GET /v1/videos/:video_id handler = %q, want VideosRetrieve", route.Handler)
		}
		return
	}

	t.Fatal("GET /v1/videos/:video_id route not registered")
}
